-- Migration: Fix POS security and concurrency locks
-- Description: Enforces strict orders RLS isolation per tenant JWT, and removes tenant-level locking from checkout.

-- 1. Eliminar las políticas de Bypass inseguras que permitían accesos públicos anónimos
...
-- 2. Crear políticas de aislamiento multi-tenant seguras basadas en JWT
CREATE POLICY "isolation_orders" ON public.orders
FOR ALL
TO authenticated
USING (tenant_id = public.current_tenant_id())
WITH CHECK (tenant_id = public.current_tenant_id());

CREATE POLICY "isolation_order_items" ON public.order_items
FOR ALL
TO authenticated
USING (tenant_id = public.current_tenant_id())
WITH CHECK (tenant_id = public.current_tenant_id());

-- 3. Re-crear complete_pos_checkout eliminando el bloqueo pg_advisory_xact_lock a nivel tenant
...
BEGIN
  ...
  -- [OPTIMIZATION] Removido pg_advisory_xact_lock a nivel tenant completo para evitar contención de bloqueos.
  -- Dejamos que Postgres gestione la concurrencia a través del bloqueo de fila 'FOR UPDATE' sobre los niveles de stock.
  ...
  LOOP
    ...
    -- Bloqueo de fila fino para concurrencia segura sin interrupciones entre cajeros
